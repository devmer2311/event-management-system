...
export async function createEvent({ userId, event, path }: CreateEventParams) {
  try {
    ...
    const organizer = await findUser(user.userId);
...
export async function getEventsByUser({
  ...
  try {
    ...
    const mongoUser = await findUser(user.userId);
    if (!mongoUser) {
      throw new Error("User not found");
    }
    
    // Use the MongoDB user's _id
    const conditions = { organizer: mongoUser._id };
